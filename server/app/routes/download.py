...
async def download_summary_get(
    ...
    summaries_response = (
        supabase_client.table("summaries").select("*").in_("id", summary_ids).execute()
    )
    ...
    for summary_data in summaries_response.data:
        ...
        summary = {
            ...
            "content": summary_data.get(
                "body", ""
            ),  # Note: 'body' in DB, 'content' in Summary type
...
async def download_questions_get(
    ...
    questions_response = (
        supabase_client.table("questions").select("*").in_("id", question_ids).execute()
    )
...
async def download_grade_get(
    ...
    try:
        ...
        messages_response = (
            supabase_client.table("messages").select("*").eq("chat", chat_id).execute()
        )
        ...
        # Get grade data
        grade_response = (
            supabase_client.table("grades").select("*").eq("id", grade_id).execute()
        )
        ...
        logger.info(
            f"File exists check: {os.path.exists(filepath) if filepath else 'No filepath returned'}"
        )

        if not filepath or not os.path.exists(filepath):
            raise HTTPException(
                status_code=500, detail=f"Failed to generate file at {filepath}"
            )
