...
async def handle_chat(
    request: Request, chat_id: str = Form(...), message_id: str = Form(...)
):
    ...
    try:
        ...
        past_messages = [
            (msg["id"], msg["bare_question"], msg.get("bare_response", ""))
            for msg in messages
        ...
        for msg in messages:
            if msg["id"] != message_id:
                ...
                past_references_for_msg, _, _ = await get_mapped_references(
                    supabase_client, f_ids, d_ids, all_file_ids, all_document_ids
                )
        ...
        (
            ...
            all_references,
            mapped_references,
        ) = await get_mapped_references(
            supabase_client, file_ids, document_ids, all_file_ids, all_document_ids
        )
        ...
        ) = await get_mapped_outcomes(supabase_client, class_id, outcomes)
        ...
        async def update_trace_id(chat_id: str, trace_id: str):
            supabase_client.table("chats").update({"trace": trace_id}).eq(
                "id", chat_id
            ).execute()
        ...
        async def update_end_agent(message_id: str, end_agent: ChatAgents):
            supabase_client.table("messages").update({"end_agent": end_agent}).eq(
                "id", message_id
            ).execute()
        ...
        try:
            ...
            supabase_client.table("messages").update(
                {"generation_status": "complete", "generation_error": ""}
            ).eq("id", message_id).execute()
