...
class FileExtractor:
    ...
    def extract_file(
        self,
        file_path: str,
        file_type: Literal["pdf", "audio", "video", "image", "other"],
    ...
    def extract_pdf(
        ...
        try:
            ...
            for i, page in enumerate(pdf):
                ...
                chunks.append(
                    FileExtractChunk(
                        text=text, page=i + 1, image_data=img_data, type="pdf_page"
                    )
                )
    ...
    def extract_audio_or_video(
        ...
        try:
                ...
                try:
                    ...
                    if progress_callback:
                        progress_callback(
                            5.0,
                            "analyzing",
                            f"Analyzing {duration:.1f} second {'video' if is_video else 'audio'}",
                        )
                    ...
                    for i in range(num_chunks):
                        ...
                        if progress_callback:
                            ...
                            progress_callback(
                                progress,
                                "splitting",
                                f"Splitting chunk {i + 1}/{num_chunks}",
                            )
                        ...
                        try:
                            ...
                            logger.info(
                                f"Created chunk directly at permanent path: {permanent_chunk_path}"
                            )

                            # Verify the file exists and has content
                            if (
                                os.path.exists(permanent_chunk_path)
                                and os.path.getsize(permanent_chunk_path) > 0
                            ):
                                logger.info(
                                    f"Verified chunk file exists: {permanent_chunk_path} ({os.path.getsize(permanent_chunk_path)} bytes)"
                                )
                            else:
                                logger.error(
                                    f"Chunk file missing or empty: {permanent_chunk_path}"
                                )
                            ...
                            if torch.cuda.is_available():
                                ...
                                logger.info(
                                    f"Cleared CUDA cache before transcribing chunk {i + 1}/{num_chunks}"
                                )
                            ...
                            img_data = None
                            if is_video:
                                img_data = self._extract_video_frame(
                                    str(permanent_chunk_path), 0
                                )
                            ...
                            chunk = FileExtractChunk(
                                ...
                                video_chunk_path=str(permanent_chunk_path)
                                if is_video
                                else None,
                                audio_chunk_path=str(permanent_chunk_path)
                                if not is_video
                                else None,
                            )
                            chunks.append(chunk)
                            ...
                            logger.error(
                                f"Error processing chunk {i + 1}/{num_chunks}: {str(e)}"
                            )

                    # Final progress update
                    if progress_callback:
                        progress_callback(
                            100.0, "complete", f"Extracted {len(chunks)} chunks"
                        )
            ...
            chunk = FileExtractChunk(
                text=f"Transcription failed. Error: {str(e)}", page=1, type="error"
            )
            chunks.append(chunk)
    ...
    def _extract_video_frame(self, video_path: str, timestamp: float) -> bytes:
        ...
        try:
            ...
            cmd = [
                ...
                "1",
                "-q:v",
                "5",  # Medium quality for smaller size
                "-f",
                "image2",
    ...
    def _write_audio_slice(
        self, src: str, start: float, end: float, dest_dir: Path, index: int
    ) -> Path:
        """
    ...
    def _generate_audio_waveform(
        ...
        try:
            ...
            if duration <= 0:
                ...
                cmd = ["ffmpeg", "-y", "-i", audio_path, "-ac", "1", temp_audio_path]
            else:
                cmd = [
                    "ffmpeg",
                    "-y",
    ...
    def extract_image_or_other(
        ...
        try:
            ...
            if is_image:
                ...
                if progress_callback:
                    progress_callback(
                        100.0, "complete", f"Extracted {len(img_data)} bytes"
                    )
                ...
                logger.info(
                    f"Created image chunk: type={chunk.type}, image_data_size={len(chunk.image_data) if chunk.image_data else 0}"
                )
                return [chunk]
            elif is_text_file:
                ...
                try:
                    ...
                    img_data = self._generate_text_image(
                    ...
                    chunk = FileExtractChunk(
                        text=text_content, page=1, image_data=img_data, type="text"
                    )

                    logger.info(
                        f"Created text chunk: type={chunk.type}, image_data_size={len(chunk.image_data) if chunk.image_data else 0}"
                    )
                    return [chunk]
                except Exception as e:
                    ...
                    try:
                        ...
                        chunk = FileExtractChunk(text=text_content, page=1, type="text")
                        logger.info(
                            f"Created text chunk: type={chunk.type}, image_data_size={len(chunk.image_data) if chunk.image_data else 0}"
                        )
                        return [chunk]
                        ...
                        chunk = FileExtractChunk(text="", page=1, type="text")
                        logger.info(
                            f"Created text chunk: type={chunk.type}, image_data_size={len(chunk.image_data) if chunk.image_data else 0}"
                        )
                        return [chunk]
                ...
                logger.info(
                    "File not detected as image or text, creating 'other' chunk"
                )
                # For other file types, just create an empty chunk
                chunk = FileExtractChunk(text="", page=1, type="other")
                logger.info(
                    f"Created other chunk: type={chunk.type}, image_data_size={len(chunk.image_data) if chunk.image_data else 0}"
                )
                return [chunk]
