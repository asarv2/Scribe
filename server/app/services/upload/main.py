...
class FileProcessor:
    ...
    async def process_uploaded_file(
        self, file_path: str, filename: str, class_id: str, file_id: str
    ) -> Tuple[bool, str]:
        """
        ...
        try:
            ...
            logger.info(
                f"Starting to process file: {filename} (ID: {file_id}, Type: {os.path.splitext(filename)[1]})"
            )
            self.saver.save_file_metadata(
                file_id,
                {
            ...
            file_response = (
                self.supabase.table("files").select("*").eq("id", file_id).execute()
            )
            ...
            logger.info("Created persistent directories for processing")
            ...
            def compression_progress_callback(
                ...
                logger.info(
                    f"Compression progress: {progress:.1f}% - {stage} {message}"
                )
            ...
            logger.info(
                f"Compression result: path={compression_result.file_path}, size={compression_result.file_size}, length={compression_result.file_length}"
            )
            ...
            for i, result in enumerate(results):
                logger.info(
                    f"Saving document {i + 1}/{total_chunks}: type={result.type}, has_image={bool(result.image_data)}"
                )

                # Verify paths exist before saving
                if result.video_chunk_path and not os.path.exists(
                    result.video_chunk_path
                ):
                    logger.error(
                        f"Video chunk path does not exist: {result.video_chunk_path}"
                    )
                if result.audio_chunk_path and not os.path.exists(
                    result.audio_chunk_path
                ):
                    logger.error(
                        f"Audio chunk path does not exist: {result.audio_chunk_path}"
                    )
                ...
                def document_progress_callback(progress, stage, message):
                    if (
                        self.current_file_id == file_id
                    ):  # Only update if we're still processing the same file
                        # Scale progress to fit within the processing stage (80-90%)
                        overall_progress = (
                            80.0
                            + (progress / 100.0 * 10.0 / total_chunks)
                            + (i / total_chunks * 10.0)
                        )
                        self.saver.save_file_metadata(
                            file_id,
                        ...
                        logger.info(
                            f"Document processing progress: {overall_progress:.1f}% - {stage} {message}"
                        )

                doc_id = self.saver.save_document(
                    class_id,
                    file_id,
                    result,
            ...
            async def run_non_cuda_tasks():
                ...
                supabase_task = asyncio.create_task(
                    asyncio.to_thread(
                        self.saver.save_file_to_supabase,
                        class_id,
                        file_id,
                ...
                results = await asyncio.gather(
                    gemini_task, supabase_task, return_exceptions=True
                )
            ...
            try:
                ...
                if "compressed_file_path" in locals():
                    ...
                    if os.path.exists(persist_dir):
                        ...
                        logger.info(
                            "Note: keeping persistent chunks at %s", persist_dir
                        )
                else:
                    logger.debug(
    ...
    async def _extract_file_content(
        ...
        def extraction_progress_callback(progress: float, status: str, message: str):
            ...
            if progress > self.current_extraction_progress or status == "error":
                self.current_extraction_progress = (
                    progress if status != "error" else self.current_extraction_progress
                )
                self.saver.save_file_metadata(
        ...
        if file_type in ["audio", "video"]:
            ...
            try:
                if torch.cuda.is_available():
                    ...
                    async with FileProcessor._cuda_semaphore:
                        ...
                        try:
                            ...
                            return await self._run_whisper_in_subprocess(
                                file_path, file_type, extraction_progress_callback
                            )
                        except Exception as e:
                            logger.error(
                                f"GPU transcription failed: {str(e)}, falling back to CPU"
                            )
                            return await self._extract_with_cpu_fallback(
                                file_path, file_type, extraction_progress_callback
                            )
                        finally:
                ...
                extraction_progress_callback(
                    0.0, "error", f"Transcription failed: {str(e)}"
                )
    ...
    async def _run_whisper_in_subprocess(
        self, file_path: str, file_type: str, progress_callback=None
    ) -> List[FileExtractChunk]:
    ...
    async def _run_extraction_directly(
        self, file_path: str, file_type: str, progress_callback=None
    ) -> List[FileExtractChunk]:
        ...
        if is_video:
            try:
                ...
                if progress_callback and self.current_extraction_progress < 15.0:
                    progress_callback(
                        15.0, "extracting_frame", "Extracting video frame"
                    )
        ...
        if segments:
            ...
            for segment in segments:
                if (
                    ...
                    or segment.get("end", 0) - current_segment["start"] > 30
                ):
            ...
            for i, segment in enumerate(merged_segments):
                ...
                if is_video and i > 0:
                    try:
                        ...
                        logger.error(
                            f"Could not extract frame at {timestamp}s: {str(e)}"
                        )
        ...
        if progress_callback and self.current_extraction_progress < 100.0:
            progress_callback(
                100.0, "complete", f"Transcription complete: {len(chunks)} segments"
            )

        return chunks

    async def _extract_with_cpu_fallback(
        self, file_path: str, file_type: str, progress_callback=None
    ) -> List[FileExtractChunk]:
            ...
            try:
                ...
                if is_video:
                    try:
                        ...
                        logger.error(
                            f"Could not extract frame in CPU fallback: {str(e)}"
                        )
                ...
                return [
                    FileExtractChunk(
                        text=result.get(
                            "text", "CPU fallback transcription (limited quality)"
                        ),
                        page=1,
                        start_time=0,
                        end_time=result.get("duration", 0),
