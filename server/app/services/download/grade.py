from app.services.download.models import Grade
import os
from pylatex import Document, Section, Command, Package
from pylatex.utils import NoEscape
import re
from app.extensions import GRADES_DIR
import logging

logger = logging.getLogger(__name__)

class GradeDownloader:
    def __init__(self, grade: Grade):
        self.grade = grade

    def download_text(self):
        """Download grade as text file"""
        name = self.grade['title']
        results = "\n".join(self.grade['results'])
        feedback = "\n".join(self.grade['feedback'])
        content = results + "\n\n" + feedback
        
        # Create directory if it doesn't exist
        os.makedirs(os.path.join(GRADES_DIR, self.grade['id']), exist_ok=True)
        
        filename = f"{name}.txt"
        filepath = os.path.join(GRADES_DIR, self.grade['id'], filename)
    ...
    def download_pdf(self):
        """Download grade as PDF file"""
        name = self.grade['title']
        
        # Create directory if it doesn't exist
        grade_dir = os.path.join(GRADES_DIR, self.grade['id'])
        ...
        success = self.save_grade_pdf(grade_dir, base_filename, title=self.grade['title'])
        
        if success:
            filepath = os.path.join(grade_dir, f"{base_filename}.pdf")
    ...
    def download_latex(self):
        ...
        success = self.save_grade_pdf(grade_dir, base_filename, title=self.grade['title'], pdf=False)
        
        if success:
            return os.path.join(grade_dir, f"{base_filename}.tex")
        return None
    ...
    def save_grade_pdf(self, directory: str, base_filename: str, title: str, pdf: bool = True):
        """
        Save grade to a LaTeX PDF file using PyLaTeX with results and feedback.
        """
        geometry_options = {
            "margin": "1in",
            "headheight": "14pt",
        ...
        doc.preamble.append(NoEscape(r'''
            ...
            % Define a command for feedback text
            \newcommand{\feedback}[1]{\textcolor{ForestGreen}{#1}}
        '''))
        ...
        with doc.create(Section('Grade Results')):
            doc.append(NoEscape(r'\begin{enumerate}'))
            
            # Process each result and feedback pair
            for i, (result, feedback) in enumerate(zip(self.grade['results'], self.grade['feedback'])):
                # Process the result with proper LaTeX formatting
                cleaned_result = self._clean_content(result)
                cleaned_feedback = self._clean_content(feedback)
                
                doc.append(NoEscape(f'\\item \\textbf{{Result:}} {cleaned_result}'))
                doc.append(NoEscape(f'\\feedback{{Feedback:}} {cleaned_feedback}'))
