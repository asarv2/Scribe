...
import io, math, zipfile, re, os, httpx, asyncio, logging
from app.extensions import FIGURES_DIR
import subprocess, shlex, textwrap, pathlib
...
PREAMBLE_RE = re.compile(
    ...
    r'|definecolor|newcommand|renewcommand|Declare)\b', re.I)
...
class FigureDownloader:
    ...
    def combine_pdf(self) -> tuple[str, str]:
        ...
        for f in self.figures:
            ...
            pre = [ln for ln in pre
                if not re.match(r'\\(documentclass|begin{document}|end{document})', ln.strip())]
            preamble.update(pre)
            figures.append(self._figure_block("\n".join(body), f["title"]))

        tex_string = self._minimal_doc(
            preamble_code="\n".join(sorted(set(preamble))),
            body_code="\n".join(figures)
        )
        ...
        proc = subprocess.run(cmd, cwd=out, stdout=subprocess.PIPE,
                            stderr=subprocess.STDOUT, text=True)
        if proc.returncode != 0:
            # grab only the '!' error lines for the log
            err_lines = [l for l in proc.stdout.splitlines() if l.lstrip().startswith("!")]
