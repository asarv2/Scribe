from agents import AgentHooks, RunContextWrapper, Agent, ToolsToFinalOutputResult
...
class FigureHooks(AgentHooks):
    def __init__(self):
        ...
        self.create_figure_tool = function_tool(
            create_figure, name_override="create_figure"
        )
        self.create_figures_tool = function_tool(
    ...
    async def on_handoff(
        ...
        self.supabase_client.table("messages").update(
            {"status_text": "Getting ready to create figures..."}
...
def upload_with_retry(bucket: str, path_on_bucket: str, local_file: str, mime: str):
    ...
    try:
        ...
        try:
            ...
            logger.info("Successfully re-uploaded file after deletion")
...
def convert_pdf_to_images(
    pdf_file: str, output_dir: str, timeout: int = 30
        ...
        try:
            run_cmd(
                [
                    ...
                    "-transp",
                    pdf_file,
                    os.path.join(output_dir, "figure"),
                ],
                timeout=timeout,
            )
...
async def create_figure_check(
    ...
    if final_tool.name == "create_figure":
        ...
        is_final = (
            isinstance(final_output_raw, CreateFigureResponse)
...
async def create_figures(
    wrapper: RunContextWrapper[Documents], figures: List[Figure]
    ...
    for figure in figures:
        ...
        try:
            ...
            # Get references
            references = [
                wrapper.context.references.get(ref, None) for ref in figure.references
            ]
            references = [
                ref.get("id")
            ...
            logger.info("LaTeX code passed pre-flight validation")
            ...
            if (
                os.path.exists(svg_output_path)
                and os.path.exists(png_output_path)
                and os.path.exists(tex_path)
            ):
                logger.info(
                    f"Using cached figure from {svg_output_path} and {png_output_path}"
                )
                # Upload cached files
                ...
                try:
                    ...
                    logger.info("Updated figure record with cached data")
            ...
            with tempfile.TemporaryDirectory() as tmpdir:
                ...
                has_pgfplots = (
                    "\\begin{axis}" in latex_code
                    or "\\begin{semilogxaxis}" in latex_code
                    or "\\begin{semilogyaxis}" in latex_code
                    or "\\begin{loglogaxis}" in latex_code
                )
                has_algorithm = "\\begin{algorithm}" in latex_code
                has_forest = "\\begin{forest}" in latex_code

                logger.info(
                    f"LaTeX content detection: tikz={has_tikz}, pgfplots={has_pgfplots}, algorithm={has_algorithm}, forest={has_forest}"
                )
                ...
                if "\\documentclass" in cleaned_latex_code and has_tikz:
                    ...
                    try:
                        ...
                        tikz_end = cleaned_latex_code.find("\\end{tikzpicture}") + len(
                            "\\end{tikzpicture}"
                        )
                        ...
                        if tikz_start >= 0 and tikz_end > tikz_start:
                            ...
                            logger.info(
                                f"Extracted tikzpicture content: {cleaned_latex_code[:50]}..."
                            )
                        else:
                            logger.warning(
                                "Could not extract tikzpicture content, using full code"
                            )
                    except Exception as extract_error:
                        logger.error(
                            f"Error extracting tikzpicture content: {str(extract_error)}"
                        )
                ...
                if auto_libs:
                    logger.info(
                        f"Auto-detected required TikZ libraries: {', '.join(auto_libs)}"
                    )
                ...
                with open(tex_file, "w") as f:
                    ...
                    f.write(
                        "\\usetikzlibrary{arrows.meta,positioning,shapes,fit,calc,decorations.pathreplacing,decorations.markings,patterns,angles,quotes"
                    )
                ...
                with open(
                    os.path.join(cache_folder, f"{code_hash}_latex_stdout.log"), "w"
                ) as f:
                    f.write(latex_result.stdout)
                with open(
                    os.path.join(cache_folder, f"{code_hash}_latex_stderr.log"), "w"
                ) as f:
                ...
                if latex_result.returncode != 0 or not os.path.exists(pdf_file):
                    logger.error(
                        f"LaTeX compilation failed with return code {latex_result.returncode}"
                    )
                    ...
                    alt_debug_tex_path = os.path.join(
                        cache_folder, f"{code_hash}_alt_debug.tex"
                    )
                    shutil.copy(alt_tex_file, alt_debug_tex_path)
                    logger.info(
                        f"Saved debug copy of alternative LaTeX file to {alt_debug_tex_path}"
                    )
                    ...
                    try:
                        ...
                        logger.info(
                            f"Alternative LaTeX compilation took {alt_compile_time:.2f} seconds"
                        )
                        ...
                        with open(
                            os.path.join(
                                cache_folder, f"{code_hash}_alt_latex_stdout.log"
                            ),
                            "w",
                        ) as f:
                        ...
                        with open(
                            os.path.join(
                                cache_folder, f"{code_hash}_alt_latex_stderr.log"
                            ),
                            "w",
                        ) as f:
                        ...
                        if alt_error_match:
                            ...
                            logger.error(
                                f"Alternative LaTeX error message: {alt_latex_error}"
                            )

                        if alt_latex_result.returncode != 0 or not os.path.exists(
                            alt_pdf_file
                        ):
                            raise subprocess.CalledProcessError(
                                alt_latex_result.returncode, "latexmk"
                            )
                        ...
                        logger.error(
                            f"Alternative LaTeX compilation also failed: {str(alt_compile_error)}"
                        )
                ...
                if svg_file is None and png_file is None:
                    ...
                    # Update DB with error status
                    supabase_client.table("figures").update(
                        {"generation_status": "error", "generation_error": error_msg}
                    ).eq("id", figure_id).execute()
                ...
                if svg_file:
                    logger.info(
                        f"Uploading SVG to Supabase storage: {class_id}/{figure_id}.svg"
                    )
                    upload_with_retry(
                        "figures",
                        f"{class_id}/{figure_id}.svg",
                ...
                if png_file:
                    logger.info(
                        f"Uploading PNG to Supabase storage: {class_id}/{figure_id}.png"
                    )
                    upload_with_retry(
                        "figures", f"{class_id}/{figure_id}.png", png_file, "image/png"
                    )
                else:
                ...
                logger.info(
                    f"Figure generation completed successfully in {figure_time:.2f} seconds"
                )
                responses.append(
                    CreateFigureResponse(
                        success=True,
            ...
            if figure_id:
                try:
                    ...
                    logger.error(
                        f"Failed to update figure error status: {str(update_error)}"
                    )
            else:
...
async def create_figure(
    wrapper: RunContextWrapper[Documents], figure: Figure
) -> CreateFigureResponse:
    """Generates a figure object given the latex code that will produce the figure. This tool is ONLY for LaTeX/TikZ code, NOT for Python/matplotlib code.
