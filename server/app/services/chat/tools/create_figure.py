...
async def create_figure(wrapper: RunContextWrapper[Documents], title: str, latex_code: str, references: List[int] = []) -> str:
    ...
    try:
        ...
        if not figure_response.data:
            logger.error(f"No generating figures found for message {message_id}")
            raise Exception(f"No generating figures found for message {message_id}")
            
        figure_id = figure_response.data[0]['id']
        logger.info(f"Found figure ID: {figure_id}")
        
        try:
            fig_number = wrapper.context.figures.index(figure_id) + 1
            logger.info(f"Figure number: {fig_number}")
        except ValueError as e:
            logger.error(f"Figure ID {figure_id} not found in context.figures: {wrapper.context.figures}")
            fig_number = len(wrapper.context.figures) + 1
            logger.info(f"Using fallback figure number: {fig_number}")
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            if latex_result.returncode != 0 or not os.path.exists(pdf_file):
                ...
                if alt_latex_result.returncode != 0 or not os.path.exists(alt_pdf_file):
                    ...
                    raise Exception(f"Figure generation failed: {error_msg}")
