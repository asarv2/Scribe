...
class BaseProcessor:
    ...
    def save_slide_latex(self, lecture_name: str, slides: dict, figures_dict: dict):
        ...
        # Add packages
        for pkg in ['hyperref', 'enumitem', 'fancyhdr', 'xcolor', 'url', 'breakurl', 'amsmath', 'amssymb', 'mathtools', 'amsthm', 'thmtools', 'float', 'subcaption']:
            doc.packages.append(Package(pkg))

        # Add caption package with styling options
        ...
        for slide_number, slide_data in slides.items():
            with doc.create(Section(f'Slide {slide_number}')):
                ...
                figures = figures_dict.get(str(slide_number), [])
                ...
                for idx, figure in enumerate(figure_list):
                    ...
                    # Begin subfigure
                    doc.append(NoEscape(r'\begin{subfigure}[b]{0.15\textwidth}'))
                    doc.append(NoEscape(r'\centering'))
                    # Use full width for the image inside the subfigure
                    doc.append(NoEscape(r'\includegraphics[width=\textwidth]{' + figure_path + '}'))
                    doc.append(NoEscape(r'\caption{' + figure['description'] + '}'))
                    doc.append(NoEscape(r'\end{subfigure}'))

                    # Add horizontal space if not the last subfigure
                    if idx < len(figure_list) - 1:
                        doc.append(NoEscape(r'\hspace{0.05\textwidth}'))
    ...
    def save_course_summary_latex(self, categorized_results: dict):
        """
        Save processed summary to a LaTeX PDF file using PyLaTeX.
        """
        geometry_options = {
