...
def test_create_directories(tmp_path, monkeypatch, use_real_extensions):
    ...
    dirs = [
        'UPLOAD_FOLDER', 'MODEL_CACHE_DIR', 'FIGURES_DIR',
        'QUESTIONS_DIR', 'SUMMARIES_DIR', 'GRADES_DIR', 'CHUNKS_DIR'
    ]
    for name in dirs:
...
def test_initialize_clients(monkeypatch, use_real_extensions):
    ...
    class MockSupabase:
        ...
        @staticmethod
        def ClientOptions():
            return type('MockClientOptions', (), {'replace': lambda self, schema=None: {'schema': schema}})()
    ...
    # Mock the imports
    monkeypatch.setitem(sys.modules, "supabase.client", type('module', (), {
