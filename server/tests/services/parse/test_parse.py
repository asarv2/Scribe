...
async def test_parse_syllabus(mock_supabase, mock_gemini, mock_runner):
    ...
    # Act
    with (
        patch("app.services.parse.parse.get_gemini", return_value=mock_gemini),
...
@pytest.mark.asyncio
async def test_parse_syllabus_with_existing_data(
    mock_supabase, mock_gemini, mock_runner
):
    ...
    # Create the FileParser instance
    parser = FileParser(
        supabase_client=mock_supabase, class_id=class_id, file_id=file_id
    )
