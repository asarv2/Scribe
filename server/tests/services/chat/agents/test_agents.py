...
# ────────────────────────────────────────────────────────────────
# ultra-thin fakes for external SDKs
# ────────────────────────────────────────────────────────────────
class _FakeLiteLLM:
    """Only needs a .model attribute for emit_google_cache()."""
    def __init__(self) -> None:
        self.model = "fake-llm"


class _FakeGemini:
    """Never called — just a placeholder object."""
    pass
...
def _patch_agent_module(
    ...
    *,
    ...
    """Replace get_litellm/get_gemini/emit_google_cache in <module_name>."""
    mod = import_module(module_name)

    # 1⃣  Fake model-fetch helpers
    monkeypatch.setattr(mod, "get_litellm", lambda: _FakeLiteLLM(), raising=True)
    monkeypatch.setattr(mod, "get_gemini", lambda: _FakeGemini(),   raising=True)

    # 2⃣  emit_google_cache decides the branch we want to test
    def _emit_cache(
        chat_id: str,
        model_name: str,
        sys_prompt: str,
...
def test_agent_builds(monkeypatch: pytest.MonkeyPatch,
    ...
    if not cached and has_tools:
        ...
        assert not getattr(built_agent, "tools", []), "Cached path should not expose tools"

    # 2) handoff()
    handoff_obj = agent_builder.handoff(built_agent)

    assert handoff_obj.agent_name.startswith(cls_name.split("Agent")[0]), "handoff agent_name mismatch"
    # must expose a callable on_invoke_handoff
    assert callable(getattr(handoff_obj, "on_invoke_handoff")), "handoff missing callback"
