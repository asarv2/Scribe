...
def guardrail_agent(mock_gemini):
    ...
    with patch("app.services.chat.agents.guardrail.get_gemini", return_value=mock_gemini):
...
async def test_input_guardrail_in_scope(guardrail_agent, mock_runner):
    ...
    with patch("app.services.chat.agents.guardrail.Runner.run", mock_runner.run):
        ...
        async def input_guardrail_function(ctx, agent, input):
            ...
            if guardrail_agent.update_chat_title:
                trace_id = await guardrail_agent.update_chat_title(
    ...
    guardrail_agent.update_chat_title.assert_awaited_once_with("chat-123", "Testing Concepts")
...
async def test_input_guardrail_out_of_scope(guardrail_agent, mock_runner):
    ...
    assert result.output_info["reason_out_of_scope"] == "Not related to the course content"
...
@pytest.mark.asyncio
async def test_output_guardrail_with_figure(guardrail_agent, mock_runner, mock_supabase):
    ...
    mock_process_tags.return_value = [{"role": "assistant", "content": "Here's a figure"}]
    
    # Mock the runner to return a correct response with outcomes
    mock_runner.run.return_value = MagicMock(
        final_output=AfterChatOutput(
            outcomes=[
                {"number": 1, "objectives": ["Testing"]}
            ],
            correct=True,
            reason=""
        ),
    ...
    # Act
    with patch("app.services.chat.agents.guardrail.get_supabase", return_value=mock_supabase), \
         patch("app.services.chat.agents.guardrail.Runner.run", mock_runner.run), \
         patch("app.services.chat.agents.guardrail.process_special_tags", mock_process_tags):
        ...
        async def output_guardrail_function(ctx, agent, output):
            ...
            if isinstance(output, CreateFigureResponse):
                chat_history = await mock_process_tags("", supabase, ctx.context, figure_id=output.figure_id)
            else:
        ...
        result = await output_guardrail_function(context, MagicMock(name="Figure Agent"), figure_response)
...
@pytest.mark.asyncio
async def test_output_guardrail_incorrect_response(guardrail_agent, mock_runner, mock_supabase):
    ...
    mock_runner.run.return_value = MagicMock(
        final_output=AfterChatOutput(
            outcomes=[],
            correct=False,
            reason="The response contains factual errors"
        ),
        final_output_as=lambda _: AfterChatOutput(
            outcomes=[],
            correct=False,
    ...
    with patch("app.services.chat.agents.guardrail.get_supabase", return_value=mock_supabase), \
        ...
        result = await output_guardrail_function(context, MagicMock(name="Content Agent"), text_response)
...
@pytest.mark.asyncio
async def test_output_guardrail_with_summary(guardrail_agent, mock_runner, mock_supabase):
    ...
    # Mock the process_special_tags function
    mock_process_tags = AsyncMock()
    mock_process_tags.return_value = [{"role": "assistant", "content": "Here's a summary"}]
    
    # Mock the runner to return a correct response with outcomes
    mock_runner.run.return_value = MagicMock(
        final_output=AfterChatOutput(
            outcomes=[
                {"number": 2, "objectives": ["Mocking"]}
            ],
            correct=True,
            reason=""
        ),
    ...
    with patch("app.services.chat.agents.guardrail.get_supabase", return_value=mock_supabase), \
        ...
        async def output_guardrail_function(ctx, agent, output):
            ...
            if isinstance(output, CreateSummaryResponse):
                chat_history = await mock_process_tags("", supabase, ctx.context, summary_id=output.summary_id)
            else:
                chat_history = [{"role": "assistant", "content": str(output)}]
        ...
        result = await output_guardrail_function(context, MagicMock(name="Summary Agent"), summary_response)
