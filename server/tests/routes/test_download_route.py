...
@pytest.mark.parametrize(
    "zip_flag,expected_type,expected_name",
    [
        (False, "image/png", "out.png"),
        (True, "application/zip", "out.zip"),
    ],
)
def test_figure_png_variants(
    client, mock_supabase, monkeypatch, tmp_path, zip_flag, expected_type, expected_name
):
    ...
    if zip_flag:
        ...
        monkeypatch.setattr(
            "app.routes.download_route.FigureDownloader.zip_pngs", mock_zip_pngs
        )
    else:
        ...
        monkeypatch.setattr(
            "app.routes.download_route.FigureDownloader.combine_pngs", mock_combine_pngs
        )
...
def test_figure_all_formats(client, mock_supabase, monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("app.services.download.figures.FigureDownloader", DummyFig)
...
def test_summary_chat_not_found(client, mock_supabase):
    r = client.get(
        "/summary",
        params=[("summary_ids", "s1"), ("format", "pdf"), ("chat_id", "no-such-chat")],
    )
    assert r.status_code == 404
    assert r.json()["detail"] == "Chat not found"
...
@pytest.mark.parametrize(
    "zip_flag,fmt,exp_type,uses_zip_method",
    [
...
def test_summary_variants(
    ...
    mock_supabase.table("chats").insert(
        {"id": "chatSum", "class": "clsSum", "name": "MyChat"}
    ).execute()
    ...
    mock_supabase.table("files").insert(
        {"id": "file1", "type": "pdf", "title": "Test File"}
    ).execute()
    ...
    monkeypatch.setattr("app.services.download.summaries.SummaryDownloader", DummySum)
...
def test_questions_variants(
    client, mock_supabase, monkeypatch, tmp_path, zip_flag, fmt, exp_type, method
):
    # prep DB
    ...
    # dummy downloader
    class DummyQ:
        def __init__(self, qdata, title, dir_id):
            pass
        ...
        def zip_latexs(self, title):
            p = tmp_path / "zlq.zip"
            p.write_bytes(b"")
            return str(p), "zlq.zip"

    monkeypatch.setattr("app.services.download.questions.QuestionsDownloader", DummyQ)
