...
def test_chat_not_found(client):
    ...
    """
    resp = client.post(
        "/message", data={"chat_id": "no-such-chat", "message_id": "msg-1"}
    )
...
def test_message_not_found(client, mock_supabase):
    ...
    resp = client.post(
        "/message", data={"chat_id": "chatX", "message_id": "missing-msg"}
    )
    assert resp.status_code == 404
...
def test_handle_chat_success(client, mock_supabase, monkeypatch):
    ...
    updated = (
        mock_supabase.table("messages").select("*").eq("id", "m1").execute().data[0]
    )
