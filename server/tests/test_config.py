...
def mock_torch():
    ...
    with patch.dict(sys.modules, {
        ...
        whisper_mock.load_model.return_value.parameters.return_value = iter([MagicMock(device=MagicMock(type="cpu"))])
...
def test_model_manager_initialization(use_real_config, tmp_path, monkeypatch):
    ...
    assert str(manager.whisper_cache_dir) == str(tmp_path / 'model_cache' / 'whisper_models')
...
def test_whisper_model_initialization(use_real_config, mock_torch, monkeypatch):
    ...
    with patch.dict(sys.modules, {'whisper': whisper_mock}):
        ...
        with patch.object(torch_mock.cuda, 'is_available', return_value=False):
            ...
            whisper_mock.load_model.assert_called_once_with("tiny.en", download_root=manager.whisper_cache_dir)
...
def test_get_whisper_model_already_initialized(use_real_config, mock_torch, monkeypatch):
    config = use_real_config
