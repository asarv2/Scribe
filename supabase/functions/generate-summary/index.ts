...
import { Summary, SummaryContent } from "./base_summary_processor.ts";
...
console.log("Starting to load function...");
...
Deno.serve(async (req) => {
  try {
    ...
    try {
      ...
      const all_lectures_response = await supabase.from("lectures").select("*").eq("class", class_id);
      ...
      if (generation_topics.length > 0) {
        ...
        names = all_lectures.filter((lecture) => generation_lectures.includes(lecture.id)).map((lecture) => lecture.name);
      }
      ...
      const lectures_processed = lectures.reduce(
        (acc, lecture_id) => {
          const lecture_name = all_lectures.find((lecture) => lecture.id === lecture_id)?.name;
          const lecture_number = all_lectures.find((lecture) => lecture.id === lecture_id)?.note_number;
      ...
      // generate 2 lectures per batch. Need to calculate this dynamically. Will do later
      const num_batches = 2
      const onBatchComplete = async (
        ...
        console.log("Generated summary for batch:", summary);
        const progress = Math.min(
          0.9,
          batchNumber / num_batches,
        );
      ...
      if (generation_lectures.length > 0) {
        ...
        summary = await lecture_summary_processor.processSummary(all_lectures, num_batches,
          onBatchComplete);
        ...
        summary = await topic_summary_processor.processSummary(all_lectures, num_batches, onBatchComplete);
