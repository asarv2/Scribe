...
export class TextbookProcessor extends BaseProcessor {
    ...
    private cleanResponse(
        ...
        if (!this.notes[textbookName]) {
            this.notes[textbookName] = {};
        }
        this.notes[textbookName][pageNumber] = cleanedResponse;
    ...
    private async processPage(
        ...
        try {
            ...
            const basePrompt = this.handwritten
                ? `Follow the 3 instructions carefully to extract the content from the textbook for the following course: ${this.courseTitle}.` +
                    ...
                    `'''}}</LATEX>\n\n` +
                    `2. Find any important figures on the pages and provide the 4 bounding box coordinates: [ymin, xmin, ymax, xmax]` +
            ...
            const additionalPrompt = this.handwritten
                ? `Use the previous textbook page's generation to help you understand the context of the current page. Remember, you should enclose everything in <LATEX> and </LATEX>, <FIGURE> and </FIGURE>, and <DESCRIPTION> and </DESCRIPTION> tags. Do not include any other formats like \`\`\`latex or \`\`\`.\n` +
                `Here is a complete example of what you should output. INPUT: PAGE 3 of 15. OUTPUT: ` 
                + `
                <LATEX>
